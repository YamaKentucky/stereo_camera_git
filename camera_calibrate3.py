...
list_left = sorted(glob('left*.jpg'))
list_right = sorted(glob('right*.jpg'))
...
#　　　　「left0.jgp」のように、ペア番号を'left','right'の後につけて同じフォルダに置く(grobが使いこなせれば直したい)
    ...
    for i in range(N):
        ...
        if found_l and found_r:
            ...
            cv2.imwrite("found_corners_right"+str(i)+".jpg", im_r)
            cv2.imwrite("found_corners_left"+str(i)+".jpg", im_l)
